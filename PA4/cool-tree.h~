...
/* Added */
class class__class;
typedef class__class* Type;
typedef SymbolTable<Symbol, class__class> Symbol_Table;
class ClassTable;
static class ClassTable* classtable;
...
public:
	class__class* father;
	int visID;			// using for inheritance loop detect;
	Symbol get_name();
	Symbol get_parent();
	bool find_method(const char* method_name, std::vector<class__class*> &parameter_list);
	Type type_check(Symbol_Table *symboltable, class__class *class_ptr);
    bool is_subclass(Type T1);
    Type least_upper_bound(Type T1);
    void gather_attribute(Symbol_Table *symboltable,class__class *class_ptr);
};
...
/**** Added ****/
public:
   virtual Type type_check(Symbol_Table* symboltable, class__class* class_ptr) = 0;
};
